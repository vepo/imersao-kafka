...
public class WeatherSensorCollector {
    public static void main(String[] args) {
        ...
        properties.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, GeolocationSerializar.class);
        properties.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, WeatherInfoSerializer.class);
        try (KafkaProducer<Geolocation, WeatherInfo> producer = new KafkaProducer<>(properties)) {
            while (running.get()) {
                try {
                    System.out.println("Sending message...");
                    WeatherInfo info = random();
                    RecordMetadata metadata = producer
                            .send(new ProducerRecord<Geolocation, WeatherInfo>("weather", info.getLocation(), info)).get();
